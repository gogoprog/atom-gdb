...
module.exports = AtomGdb = {
    ...
    activate: function(state) {
        ...
        atom.workspace.observeTextEditors(function(editor) {
            for(var i in AtomGdb.breakpoints)
            {
                var bp = AtomGdb.breakpoints[i];
                if(bp.filepath == editor.getPath())
                {
                    ...
                    AtomGdb.setupMarker(editor, marker, bp);
                }
            }
        });
    },
    ...
    runProcess: function(command, args, cwd) {
        if(this.childProcess !== null) {
            atom.notifications.addWarning("[atom-gdb] A child process is already running. (" + this.childProcess.command + ")");
            return null;
        }
    ...
    setupMarker: function(editor, marker, item) {
